...
# CAN TEST ENDPOINT
@app.post(f"{API_PREFIX}/parse/write/CAN_test")
@auth.login_required
def parse_and_write_request_to_CAN():
    """
...
def parse_and_write_request():
    ...
    for i in range(len(message.data[list(message.data.keys())[0]])):
        ...
        # write to InfluxDB bucket based on the type
        try:
            write_api.write(bucket=message.type + "_test", org=INFLUX_ORG, record=point)
            app.logger.info(
                f"Wrote '{name}' measurement to url={INFLUX_URL}, org={INFLUX_ORG}, bucket={message.type + "_test"}!")
        except Exception:
            app.logger.warning("Unable to write measurement to InfluxDB!")
            return {
                "result": "INFLUX_WRITE_FAIL",
